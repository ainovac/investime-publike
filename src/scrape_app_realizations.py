"""
Scraper for APP's "Regjistri i Realizimeve" (realized/awarded procurement register):
https://www.app.gov.al/regjistri-i-realizimeve/

Same site/pattern as the tender-notice scraper (Umbraco, session + CSRF token +
ufprt pagination walk, no JSON API - see ../../app-extract/app_gov_extract.py for
the original). This register carries winner name/NIPT, contract value and award
date - but NO ref_no/notification_no, so it cannot be cleanly joined back to a
specific already-scraped tender notice (see project chat log / CLAUDE.md). It is
therefore kept as its own independent real dataset.

Usage:
    pip install requests beautifulsoup4 lxml pandas
    python scrape_app_realizations.py                  # walk all ~1000 pages
    python scrape_app_realizations.py --max-pages 5     # quick test
"""
import argparse
import json
import re
...
URL = f"{BASE}/regjistri-i-realizimeve/"
...
# Albanian label -> our column name
RENAME = {
    "Objekti i procedurës së prokurimit": "procurement_object",
    ...
    "Vlera e kontratës": "contract_value_raw",
    "Data e prokurimit": "procurement_date",
    "Fituesi": "winner_name",
    "NIPT": "winner_nipt",
...
def parse_records(soup):
    ...
    rows = []
    for it in items:
        # The object text and the ul.list-inline are separate sibling divs inside
        # .list-group-item-heading on this page (unlike the tender-notice page),
        # so anchor on the <strong> label's own parent, not the whole heading.
...
def transform(rows):
    ...
    if "contract_value_raw" in df:
        df["contract_value"] = df["contract_value_raw"].map(clean_money)
        df.drop(columns=["contract_value_raw"], inplace=True)
    ...
    if "procurement_date" in df:
        df["procurement_date"] = pd.to_datetime(df["procurement_date"], format="%d-%m-%Y", errors="coerce")
...
if __name__ == "__main__":
    ...
    checkpoint_path = out / "app_realizations_checkpoint.json"
    ...
    df = transform(rows)
    df.to_csv(out / "app_realizations.csv", index=False, encoding="utf-8-sig")
    checkpoint_path.unlink(missing_ok=True)
    print(f"Saved {len(df)} realization records (of {total_pages} total pages seen) to {out}/")
